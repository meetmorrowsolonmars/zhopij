...
import (
	...
	"github.com/meetmorrowsolonmars/zhopij/post/internal/app/v1/post"
	"github.com/meetmorrowsolonmars/zhopij/post/internal/pkg/repositories"
...
func main() {
	...
	user := os.Getenv("POSTGRES_POST_USER")
	password := os.Getenv("POSTGRES_POST_USER_PASSWORD")
	name := os.Getenv("POSTGRES_POST_DB")
	...
	postRepository := repositories.NewPostRepository(3*time.Second, db)
	postService := services.NewPostService(postRepository)

	// grpc services setup
	postGrpcServer := post.NewPostServiceImplementation(postService, logger)

	// grpc server setup
	server := grpc.NewServer(grpc.ChainUnaryInterceptor(metrics.UnaryServerInterceptor()))

	post.RegisterGRPCServer(server, postGrpcServer)
